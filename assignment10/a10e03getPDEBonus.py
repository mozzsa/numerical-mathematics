...
integral(u`_d*v'+u_d*v,a,b)=integral(f-cw)*v,a,b)  v(a) = 0 v(b)=0
...
def a10e03getPDEBonus(a,b,c,alpha,beta,f,N):
   ...
   for i in np.arange(1,len(xh)-1):
       ...
       temp_f = h*((v_l(xh)+v_r(xh))*(f(xh)-c(xh)*w(xh)))#trapezoidal rule
       temp_f[0]= temp_f[0]/2.
       temp_f[-1]= temp_f[-1]/2. 
